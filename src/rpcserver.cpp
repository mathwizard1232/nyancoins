...
CAmount AmountFromValue(const Value& value)
{
    ...
    if (dAmount <= 0.0 || dAmount > 337000000.0)
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use nyancoind, or the -server option to nyancoin-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
              ...
              "for example: alertnotify=echo %%s | mail -s \"Nyancoin Alert\" admin@foo.com\n"),
